...
class CommentsManager:
    ...
    async def list_comments_by_book(self, book_id: str, limit: int = 100, skip: int = 0, newest_first: bool = True) -> List[CommentOut]:
        docs = await self._repo.find_by_book(book_id, limit=limit, skip=skip, newest_first=newest_first)
        return [_to_out(d) for d in docs]

    async def list_comments_by_user(self, username: str, limit: int = 100, skip: int = 0, newest_first: bool = True) -> List[CommentOut]:
